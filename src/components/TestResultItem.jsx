...
const TestResultItem = ({ result, user, onUpdate, onDelete }) => {
  ...
  return (
    <div className="p-6 my-10 bg-slate-100 rounded-lg shadow-lg text-gray">
      <div className="flex justify-between items-center border-b border-gray-700 pb-3 mb-3">
        <h4 className="text-xl font-semibold">{result.nickname}</h4>
        <p className="text-sm text-gray-500">{formattedDate}</p>
      </div>
      <p className="text-2xl font-bold text-rose-400 my-5">{result.result}</p>
      <p className="text-base text-gray-400300 mb-4">{description}</p>
      {isOwner && (
        <div className="flex justify-end space-x-4">
          <button
            onClick={handleToggleVisibility}
            className="bg-indigo-500 py-2 px-4 rounded-lg text-sm hover:bg-indigo-600 transition text-white"
          >
            {result.visibility ? "비공개로 전환" : "공개로 전환"}
          </button>
          <button
            onClick={handleDelete}
            className="bg-rose-400 py-2 px-4 rounded-lg text-sm hover:bg-rose-300 transition text-white"
          >
            삭제
          </button>
        </div>
